...
#Substitution Box for Decryotion
S_Box_D=[0xa,0x5,0x9,0xb,0x1,0x7,0x8,0xf,0x6,0x0,0x2,0x3,0xc,0x4,0xd,0xe]
#Substitution Box for Enncryption
...
def aes_decrypt(data,key):
  ...
  #Add Round Key i.e. Key2 XOR Plaintext
  state=((w[4] << 8)+w[5]) ^ data
  ...
  state=SubNib(S_Box_D,state)
  print('After Round 1 InvSubstitute Nibbles:',state)
  #Inverse AddRoundKey i.e. Key1 XOR state
  state=AddRoundKey(IntToVec((w[2]<<8)+w[3]),state)
  print('After Round 1 InvAdd Round Key:',state)
  ...
  #Add Round Key i.e. Key0 XOR state
  state=AddRoundKey(IntToVec((w[0] << 8)+w[1]),state)
